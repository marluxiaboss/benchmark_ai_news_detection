    ...
    def run_pipeline(self):
        ...
        if isinstance(scorer, RefScorer):
            ...
            if hasattr(self, "dataset_path_compare"):
                ...
                assert self.dataset_path_compare is not None, "Both datasets must be provided for Prometheus Scorer"
                
                dataset_test = concatenate_datasets([self.dataset["test"], self.dataset["eval"]])            
                dataset_test2 = concatenate_datasets([self.dataset_path_compare["test"], self.dataset_path_compare["eval"]])
