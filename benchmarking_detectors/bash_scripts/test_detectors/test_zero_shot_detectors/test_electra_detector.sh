...
# specific for bert detector
detector="bert_detector"
weights_checkpoint="detector_model_weights/electra_mistral_weights/best_model.pt"
detector_name="electra_large"
...
for i in ${!generators[@]}; do
    ...
    for j in ${!attacks[@]}; do
        attack=${attacks[$j]}
        print_current_time
        echo "Detecting text generated with generator: $generator, watermark: $watermark_scheme, attack: $attack", dataset_size: $dataset_size, experiment_name: $gen_experiment_name
        python test_detector.py detection=$detector generation=$attack generation.generator_name=$generator generation.experiment_name=$gen_experiment_name \
