...
class ExperimentTestDetectorPipeline2(ExperimentPipeline):
    def __init__(self, cfg, detector, experiment_path, dataset_experiment_path, batch_size=1):
        self.cfg = cfg
    ...
    def run_pipeline(self):
        ...
        log.info("Classifying the articles...")
        preds, logits, preds_at_threshold = self.detector.detect(fake_true_articles, detection_threshold, batch_size=self.batch_size)
        labels = dataset["label"]
        self.evaluate_detector(preds, logits, preds_at_threshold, labels, dataset, detection_threshold, eval_set="test")
