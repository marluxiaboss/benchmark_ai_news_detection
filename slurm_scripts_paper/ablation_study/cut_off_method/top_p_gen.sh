...
watermark_delta=5.0
cut_off_method="top_p"

for i in ${!top_p_values[@]}; do
    watermark_top_p_value=${top_p_values[$i]}
    experiment_name="top_p_variation_${watermark_top_p_value}"
    ...
    echo "Bias value: $watermark_delta"
    ...
    create_dataset generation=$attack watermark=$watermark_scheme generation.generator_name=$generator_name \
            ...
            watermark.cut_off_method=$cut_off_method watermark.top_p=$watermark_top_p_value
done
