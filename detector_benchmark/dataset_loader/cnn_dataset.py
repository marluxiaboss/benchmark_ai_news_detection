...
from .dataset_loader_utils import filter_duplicates, create_train_from_dataset, create_splits
...
class CNNDataLoader(FakeTruePairsDataLoader):
    def __init__(
        self,
        dataset_size: int,
        hf_dataset_path: str = "abisee/cnn_dailymail",
    ...
    def process_data(self, dataset: DatasetDict) -> DatasetDict:
        ...
        percent_filtered = (
            (nb_samples_before_filter - nb_samples_after_filter) / nb_samples_before_filter * 100
        )
        print(
        ...
        dataset_fake = dataset.map(
            lambda x: {"label": 1, self.text_field: "", "prefix": x["prefix"]}
        )
    ...
    def load_data(self) -> DatasetDict:
        ...
        processed_dataset = create_splits(
            processed_dataset,
            train_split_size_percent,
            eval_split_size_percent,
            test_split_size_percent,
        )
