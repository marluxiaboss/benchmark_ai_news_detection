...
def compute_bootstrap_metrics(data, labels, n_bootstrap=1000, flip_labels=False):
    ...
    for i in range(n_bootstrap):
        ...
        nb_false_positives[i] = np.sum(
            (data[bootstrap_sample] == 1) & (labels[bootstrap_sample] == 0)
        )
        nb_false_negatives[i] = np.sum(
            (data[bootstrap_sample] == 0) & (labels[bootstrap_sample] == 1)
        )
        nb_true_positives[i] = np.sum(
            (data[bootstrap_sample] == 1) & (labels[bootstrap_sample] == 1)
        )
        nb_true_negatives[i] = np.sum(
            (data[bootstrap_sample] == 0) & (labels[bootstrap_sample] == 0)
        )
            ...
            match metric:
                ...
                case "precision":
                    if nb_true_positives_i + nb_false_positives_i == 0:
                        ...
                        metric_results[i] = nb_true_positives_i / (
                ...
                case "recall":
                    if nb_true_positives_i + nb_false_negatives_i == 0:
                        ...
                        metric_results[i] = nb_true_positives_i / (
                            nb_true_positives_i + nb_false_negatives_i
                        )
                case "f1_score":
                    if (2 * nb_true_positives_i + nb_false_positives_i + nb_false_negatives_i) == 0:
                        ...
                        metric_results[i] = (
                            ...
                            * nb_true_positives_i
                            / (
                                2 * nb_true_positives_i
                                + nb_false_positives_i
                                + nb_false_negatives_i
                            )
                        )
                case "fp_rate":
                    if (nb_false_positives_i + nb_true_negatives_i) == 0:
                        ...
                        metric_results[i] = nb_false_positives_i / (
...
def create_logger_file(log_path):
    ...
    log = create_logger(__name__, silent=False, to_disk=True, log_file=f"{log_path}")
