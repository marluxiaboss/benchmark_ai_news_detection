...
def init_pipelines(cfg: DictConfig, log):
    ...
    if cfg.pipeline.use_bert_scorer:
        ...
        pipeline = TextQualityPipeline(
            bert_scorer,
            watermarked_dataset_path_main,
            dataset_path_compare=watermarked_dataset_path_compare,
            batch_size=cfg.pipeline.batch_size,
            return_loss_lists=cfg.pipeline.return_loss_lists,
        )
    ...
    if cfg.pipeline.use_idf_scorer:
        ...
        pipeline = TextQualityPipeline(
            idf_scorer, watermarked_dataset_path_main, batch_size=cfg.pipeline.batch_size
        )
        pipelines.append(pipeline)
    ...
    if cfg.pipeline.use_prometheus_scorer:
        ...
        assert (
            watermarking_scheme_name_main != watermarking_scheme_name_compare
        ), "Need to provide two different watermarking schemes"
        ...
        pipeline = TextQualityPipeline(
            prometheus_scorer,
            watermarked_dataset_path_main,
            watermarked_dataset_path_compare,
            batch_size=cfg.pipeline.batch_size,
        )
        pipelines.append(pipeline)
