...
class LikelihoodModelWatermarked(LikelihoodModel):
    ...
    def __init__(self, watermarking_depth: int):
        ...
        self.beta = nn.Parameter(-2.5 + 0.001 * torch.randn(1, 1, watermarking_depth))
        self.delta = nn.Parameter(0.001 * torch.randn(1, 1, watermarking_depth, watermarking_depth))
    ...
    def _compute_latents(self, g_values: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        x = g_values.unsqueeze(-2).repeat(1, 1, self.watermarking_depth, 1)
...
class BayesianDetectorModule(nn.Module):
    ...
    def __init__(self, watermarking_depth: int, baserate: float = 0.5):
        super().__init__()
        self.watermarking_depth = watermarking_depth
    ...
    def forward(
        ...
        log_likelihoods_watermarked = torch.log(torch.clamp(likelihoods_watermarked, min=1e-30))
        log_likelihoods_unwatermarked = torch.log(torch.clamp(likelihoods_unwatermarked, min=1e-30))

        log_odds = log_likelihoods_watermarked - log_likelihoods_unwatermarked

        relative_surprisal_likelihood = torch.sum(log_odds * mask, dim=(1, 2))
...
def train_model(
    ...
    for epoch in range(epochs):
        ...
        for i in range(0, len(g_values), minibatch_size):
            ...
            loss = (
                F.binary_cross_entropy(pred, batch_w.float())
        ...
        with torch.no_grad():
            if g_values_val is not None:
                ...
                val_loss = F.binary_cross_entropy(val_pred, watermarked_val.float())
    ...
    def train_best_detector(
        ...
        *,
        ...
        if torch_device.type not in ("cuda", "tpu"):
            raise ValueError("Training is unstable on CPUs. Please use GPU or TPU.")
