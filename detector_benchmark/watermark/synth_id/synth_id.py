...
class SynthIDUtils:
    ...
    def update_scores_distortionary(
        ...
        for i in range(depth):
            ...
            coeffs = torch.where(
                torch.logical_and(g_values_at_depth == 1, probs > 0), coeff_in_g, coeff_not_in_g
            )
            probs = probs * coeffs
...
class SynthIDLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        if self.state is None:
            self.state = {
                "context": torch.zeros(
                    (batch_size, self.ngram_len - 1), dtype=torch.int64, device=self.device
                ),
                "context_history": torch.zeros(
                    (batch_size, self.context_history_size), dtype=torch.int64, device=self.device
                ),
                "num_calls": 0,
            }
        ...
        self.state["context_history"] = torch.cat(
            (hash_context, self.state["context_history"]), dim=1
        )[:, :-1]
    ...
    def compute_context_repetition_mask(
        ...
        for i in range(num_contexts):
            context = contexts[:, i, :]
            hash_result = torch.ones(batch_size, device=self.device, dtype=torch.long)
            context_hash = self.utils.accumulate_hash(hash_result, context)[:, None]
...
class SynthID(BaseWatermark):
    ...
    def __init__(
        self, algorithm_config: str, gen_model, transformers_config: ModelConfig, *args, **kwargs
    ) -> None:
    ...
    def detect_watermark(self, text: str, return_dict: bool = True, *args, **kwargs):
        ...
        eos_mask = self.logits_processor.compute_eos_token_mask(
            input_ids=encoded_text, eos_token_id=self.config.generation_tokenizer.eos_token_id
        ...
        score = self.detector.detect(g_values_np, mask_np)[
            0
        ]  # Take first element as we have batch_size=1
