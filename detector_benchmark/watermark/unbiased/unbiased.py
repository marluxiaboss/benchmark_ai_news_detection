...
class UnbiasedUtils:
    ...
    def reweight_logits(
        ...
        boundary_1 = torch.argmax(
            (s_cumsum > self.config.alpha).to(torch.int), dim=-1, keepdim=True
        )
        ...
        boundary_2 = torch.argmax(
            (s_cumsum > (1 - self.config.alpha)).to(torch.int), dim=-1, keepdim=True
        )
    ...
    def score_sequence(self, input_ids: torch.LongTensor) -> tuple[float, list[int]]:
        ...
        z_score = (green_tokens - (1 - self.config.gamma) * input_ids.size(-1)) / sqrt(
            input_ids.size(-1)
        )
...
class UnbiasedWatermark(BaseWatermark):
    ...
    def __init__(
        ...
        Parameters:
            ...
            transformers_config (TransformersConfig): Configuration for the transformers model.
        """
        self.config = UnbiasedConfig(algorithm_config, gen_model, transformers_config)
